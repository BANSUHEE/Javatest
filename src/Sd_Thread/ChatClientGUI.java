		...
		createButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				try {
					...
					// 사용자가 모든 필드를 입력한 경우에만 회원가입 진행
					if (newid.isEmpty() || newPassword.isEmpty() || newName.isEmpty() || newPhone.isEmpty()) {
					...
					if (resultCnt > 0) {
						JOptionPane.showMessageDialog(signupFrame,
								"The ID is already in use. Please choose another one.", "Error",
								JOptionPane.ERROR_MESSAGE);
						return;
					}
...
// 대화내용 삭제하는 method
	...
	/*
	 * 관리자 창을 닫고 사용자 정보를 보여주는 새로운 창을 열어, 데이터베이스에서 조회한 사용자 정보를 표시하는 역할
	 */
